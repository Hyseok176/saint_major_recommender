...
*   **UI/UX 개선**:
    *   **탐색 탭 개선**: 모든 페이지의 상단 탐색 탭에 마우스를 올렸을 때 시각적 효과(hover)를 추가하여 사용자 경험을 개선했습니다.
    *   **헤더 레이아웃 변경**:
        *   모든 페이지의 헤더 레이아웃을 '로고 - 페이지 제목 - 탐색 탭' 순서로 정렬하여 일관성을 높였습니다.
        *   기존에 분리되어 있던 로고와 페이지 제목을 탐색 탭과 같은 행에 배치하여 상단 공간 활용도를 개선했습니다.
    *   **로고 이원화**:
        *   **메인 페이지**: 탐색 탭 옆에 `small.png` 로고를 사용하여 간결한 디자인을 유지했습니다.
        *   **로그인 페이지**: `unnamed.png` 로고를 사용하여 첫 화면의 주목도를 높였습니다.
...
*   **오류 수정 및 안정성 강화**:
   ...
   1. 과목 추천 알고리즘 개선: 전공 및 교양 과목 추천을 분리하고, 각 카테고리별로 미수강 과목 중 수강생 수가 많은 상위 5개 과목을 추천하도록
      로직을 변경했습니다.
   2. "전체 과목" 페이지 개선: "전체" 탭을 "비전공"으로 변경하고, 사용자의 전공과 겹치지 않는 과목만 표시되도록 필터링 기능을 추가했습니다.
   3. 파일 업로드 페이지 안내 개선: 파일 형식, 전공 선택의 중요성, 미래 전공 정보 제공, 그리고 서강대 세인트 시스템에서 파일을 다운로드하는
      방법에 대한 상세 안내를 추가했습니다.
   4. 오류 처리 개선: 파일 업로드 오류에 대한 구체적인 메시지를 제공하고, 404, 500 등 HTTP 오류에 대한 사용자 친화적인 커스텀 오류 페이지를
      구현했습니다.
   5. 수강 이력 페이지 개선: 정규 학기의 소수점(.0)을 제거하여 학기 표시를 더 깔끔하게 만들었습니다.
